...
export const getDashboardMetrics = async () => {
  ...
  const pendingQuotations = await Quotation.countDocuments({ status: { $in: ['draft', 'generated', 'sent'] } });
  ...
  const averageQuotationValue = quotationValues.length > 0 ? (quotationValues[0].totalValue / quotationValues[0].count) : 0;
  ...
  const mostSelectedPackage = mostSelectedPackageData.length > 0 ? mostSelectedPackageData[0].pkg.name : 'N/A';
...
export const getPaymentAnalytics = async () => {
  ...
  const averagePaymentValue = successfulPaymentsCount > 0 ? (totalRevenueData.total / successfulPaymentsCount) : 0;
